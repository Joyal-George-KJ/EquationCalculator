    ...
    return (
        <div className="text-white rounded-2xl shadow-2xl">
            ...
            {[
                ...
                <div
                    key={i}
                    className="mb-4 p-4 bg-neutral-700 rounded-xl shadow"
                >
                    <h3 className="text-lg font-semibold">{`Step ${
                        i + 3
                    }: ${title}`}</h3>
                    <p className="text-sm text-gray-400">
                        {values.join(" + ")}
            ...
            <div className="mb-4 p-4 bg-neutral-700 rounded-xl shadow">
                <h3 className="text-lg font-semibold">
                    Step 6: Total of Cards, Cash, Inlend, UPI
                </h3>
                <p className="text-sm text-gray-400">{`${twoDecimal(
                    cardTotal
                )} + ${twoDecimal(inlendTotal)} + ${twoDecimal(
                    cashTotal
                )} + ${twoDecimal(upiDifference)}`}</p>
            ...
            <div
                className={`p-4 rounded-xl text-center font-bold ${
                    finalDifference >= -10 ? "bg-green-600" : "bg-red-600"
                }`}
            >
                <h3 className="text-lg">Final Difference</h3>
                <p className="text-sm text-gray-400">{`${twoDecimal(
                    totalSum
                )} - ${twoDecimal(readingDifference)}`}</p>
                <p className="text-md">
                    ₹
                    {`${finalDifference > 0 ? "+" : ""}${twoDecimal(
                        finalDifference
                    )}`}
                </p>
            </div>
